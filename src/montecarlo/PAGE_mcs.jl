...
function get_page_mcs()
    ...
    mcs = @defsim begin
        ...
        # so we use an RV here because in the model this is not an explicitly
        # shared parameter, then assign below in component section
        rv(RV_tcal_CalibrationTemp) = TriangularDist(2.5, 3.5, 3.0)
        ...
        SeaLevelRise.sltemp_SLtemprise = TriangularDist(0.7, 3.0, 1.5)
        SeaLevelRise.sla_SLbaselinerise = TriangularDist(0.5, 1.5, 1.0)
